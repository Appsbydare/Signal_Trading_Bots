...
export default function ResourceArticle({ params }: { params: Params }) {
  ...
  const isValidArticle = !!article && article.status === "available" && !!content;
    ...
    <article className="space-y-10">
      {isValidArticle ? (
        ...
        <div className="space-y-4">
          ...
          <h1 className="text-2xl font-semibold text-[var(--text-main)] md:text-3xl">
            Article not available
          </h1>
          <p className="text-sm text-[var(--text-muted)] md:text-base">
            The article you tried to open is not available. Please use the Resources page to pick
            another guide.
          </p>
        </div>
      )}
